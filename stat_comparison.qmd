...
dplyr::select(-c(label, ibaq, value, sumvalue)) %>%
dplyr::distinct() %>%
dplyr::ungroup() %>%
...
```{r Stringdb upload}
gene_names <- data.table::fread(here::here("9606.protein.info.v12.0.txt")) %>% 
  dplyr::select(string_protein_id, preferred_name)

human_stringDB <- data.table::fread(here::here("9606.protein.links.detailed.v12.0.txt")) 

human_string <- human_stringDB %>%
  dplyr::mutate(
...
mn_limma<- openxlsx::read.xlsx(xlsxFile = "Univariate_table_2023-10-19.xlsx") %>% 
  dplyr::filter(mn_vs_nuc_significant) %>% 
  dplyr::filter(mn_vs_nuc_fold_change>0) #Only selected significant upregulated
...
data <-
    rba_string_interactions_network(
      ids = limma_tibble$gene_names,
      species = 9606,
      required_score = 400
    )


rescoring <- function(table){
  data <- table %>% 
    dplyr::select(source = preferredName_A, target = preferredName_B, score, escore, dscore) %>%
    dplyr::mutate(score1 = (escore - 0.041) * (1 - 0.041)) %>%
  dplyr::mutate(score2 = (dscore - 0.041) * (1 - 0.041)) %>%
