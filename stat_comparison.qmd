...
#   dplyr::mutate(
#     int = if_else(
#       source %in% mn_limma$gene_names &
#         target %in% mn_limma$gene_names,
#       1,
#       0
#     ),
#     source_target = paste0(source, "_", target),
#     target_source = paste0(target, "_", source)
#   ) %>%
#   dplyr::filter(int == 1) %>%
#   dplyr::select(-int) %>% 
#   tidyr::unite("test", protein1:protein2, remove=TRUE) %>% 
#   dplyr::distinct(test, .keep_all = TRUE) %>% View()
...
filt_mn <- mn_limma_int %>% dplyr::filter(re_score>=0.8)
...
```{r}
bah<- openxlsx::read.xlsx(xlsxFile = "Univariate_table_2023-10-25-2.xlsx") %>% 
  dplyr::select(majority_protein_i_ds,gene_names
) %>%
dplyr::mutate(majority_protein_i_ds = stringr::str_extract(majority_protein_i_ds, "[^;]*")) %>% 
  dplyr::right_join(limma_tibble, by="gene_names") %>% 
  dplyr::select(gene_names, majority_protein_i_ds) 
  


rba_string_interactions_network(ids = limma_tibble %>% pull(gene_names),
                                    species = 9606,
                                    required_score = 400) %>% 
  tidyr::unite("StringID", stringId_A:stringId_B, remove = TRUE) %>% 
  dplyr::distinct(StringID, .keep_all = TRUE) %>% 
